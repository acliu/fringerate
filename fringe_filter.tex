...
%XXX need better pitch in abstract
\begin{abstract}
...
we recover are shown in Figure \ref{fig:eff_beam}.  These compare well with the effective beam response
we predict by assigning a response to each position on the sky by looking up the filter response corresponding
to the fringe rate at that position, and multiplying this by the original primary beam, as illustrated
in Figure \ref{fig:sim_beam}.  As these simulations illustrate, the approximations we have made in our analytic
...
to within the sample noise of our simulation.
% XXX get the numbers from the simulation to be more quantitiative here.
