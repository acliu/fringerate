...
\langle \widetilde{V}_i(\mathbf{u},\eta) \widetilde{V}_j(\mathbf{u},\eta)^*\rangle \approx S \left[P^I (\mathbf{k}) \Omega_{ij}^I + P^Q (\mathbf{k}) \Omega_{ij}^Q\right],
\end{equation}
...
but with superscripts indicating the Stokes components. The Stokes $I$ power spectrum $P^I$ is the same as the
power spectrum $P$ defined in Section \ref{sec:PspecOptimization}, with the new notation serving only to
distinguish it from the Stokes $Q$ power $P^Q$. In deriving Equation \eqref{eq:LeakyCorrelFct}, we made two
...
uncorrelated. The same approximation can be made here, allowing us to define $\gamma^I$ and $\gamma^Q$ by
letting $\Omega_{ij}^I \equiv \delta_{ij} \gamma_i^I$ and $\Omega_{ij}^Q \equiv \delta_{ij} \gamma_i^Q$. With no
...
\begin{equation}
\langle \widehat{P} \rangle = \left( S \sum_i g_i \gamma^I_i \right) P^I + \left( S \sum_i g_i \gamma^Q_i \right) P^Q.
\end{equation}
If, as before, we require our weights to be normalized so that $S \sum_i g_i \gamma^I_i =1$, the first term gives an
...
\begin{equation}
\mathcal L = \sigma^4 \mathbf{g} \cdot \mathbf{g} + \left(S P^Q \right)^2 \left(\mathbf{g} \cdot \boldsymbol \gamma^Q \right)^2 - \lambda \mathbf{g} \cdot \boldsymbol \gamma^I,
\end{equation}
...
vectors $\boldsymbol \gamma^I$ and $\boldsymbol \gamma^Q$. Now, notice that if we define
\begin{equation}
\mathbf{H} \equiv \sigma^4 \mathbf{I} + \left(S P^Q \right)^2 \boldsymbol \gamma^Q (\boldsymbol \gamma^Q)^t,
\end{equation}
our expression can be written as
\begin{equation}
\mathcal L = \mathbf{g}^t \mathbf{H} \mathbf{g} - \lambda \mathbf{g}^t \boldsymbol \gamma^I.
\end{equation}
Differentiating this, setting the result to zero, and solving for the normalized weights gives
\begin{equation}
\mathbf{g} =  \frac{\mathbf{H}^{-1}  \boldsymbol \gamma^I}{ S (\boldsymbol \gamma^I)^t  \mathbf{H}^{-1}  \boldsymbol \gamma^I}.
\end{equation}
This expression involves $\mathbf{H}^{-1}$ which can be evaluated quite explicitly in our case thanks to the
Woodbury formula, giving
\begin{equation}
\mathbf{H}^{-1} = \sigma^{-4} \left( \mathbf{I} - \frac{(S P^Q / \sigma^2)^2 (\boldsymbol \gamma^Q)^t \boldsymbol \gamma^Q}}{1 + (S P^Q)^2 (\boldsymbol \gamma^Q)^t \boldsymbol \gamma^Q} \right)
\end{equation}
