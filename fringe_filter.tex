...
In XXX: cite, it was shown that estimates of the three-dimensional power spectrum of brightness temperature fluctuations can be obtained from a single baseline by Fourier transforming visibility data along the frequency axis (forming a ``delay spectrum"), and then taking the absolute square of the results. Specifically, an unbiased estimate $\widehat{P}(\mathbf{k})$ of the power spectrum can be obtained by computing
\begin{equation}
\label{eq:PspecEst}
\widehat{P}(\mathbf{k}) \approx \left( \frac{\lambda^2}{2 k_B} \right)^2 \frac{X^2 Y}{\Omega_{pp} B} |\widetilde{V} (u,v,\eta)|^2,
\end{equation}
assuming that the baseline in question is short and that the field-of-view is not too large. (For relaxations of the former assumption, see XXX: cite myself, wedge paper no. 1). Here, XXX: define all symbols.
\begin{equation}
X \equiv ,
\end{equation}
\begin{equation}
...
and
\begin{equation}
\Omega_{pp} \equiv \int d\Omega | A(\rhat) |^2.
\end{equation}
Our goal in this section is to examine how one can use fringe-rate filtered visibilities in place of raw visibilities in Equation \eqref{eq:PspecEst} to boost signal-to-noise. In particular, we will derive optimal fringe-rate weights that minimize the final error bars on the power spectrum estimate $\widehat{P}(\mathbf{k})$.
...
%repeated use of simulated data. 
